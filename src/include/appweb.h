...
extern MprModule *maUploadFilterInit(MaHttp *http, cchar *path);
...
#define MA_LOC_PUT_DELETE       0x20        /**< Support PUT|DELETE */
 ...
 *  @ingroup MaPacket
 */
extern MaPacket *maCreateHeaderPacket(MprCtx ctx);
 ...
 *  @ingroup MaPacket
 */
extern MaPacket *maCreateEndPacket(MprCtx ctx);
...
#define MA_QUEUE_STARTED        0x20        /**< Queue's start routine has been called */
#define MA_QUEUE_EOF            0x40        /**< Queue at end of data */
...
typedef void (*MaQueueStart)(struct MaQueue *q);
...
typedef struct MaQueue {
    ...
    MaQueueStart    start;                  /**< Start the queue, run after all opens have completed */
...
#define MA_STAGE_VARS           0x10000         /**< Create query and form variables table */
...
typedef struct MaStage {
    ...
    /**
     *  Start the queue
     *  @description The queue is started after all the pipline open entry points have been run.
     *  @param q Queue instance object
     *  @ingroup MaStage
     */
    void            (*start)(MaQueue *q);
...
#define MA_CONN_PIPE_CREATED        0x4     /**< Request pipeline created */
...
extern bool maMatchModified(struct MaConn *conn, MprTime time);
extern void maPrepConnection(MaConn *conn);
 ...
 */
#define MA_CHUNK_START      1               /**< Start of a new chunk */
#define MA_CHUNK_DATA       2               /**< Start of chunk data */
#define MA_CHUNK_EOF        3               /**< End of last chunk */
...
extern void         maAbortConnection(struct MaConn *conn, int code, cchar *fmt, ...);
 ...
 *  @ingroup MaResponse
 */
extern void maSetCookie(MaConn *conn, cchar *name, cchar *value, cchar *path, cchar *domain, int lifetime, bool secure);
...
extern char         *maGetDateString(MprCtx ctx, MprPath *sbuf);
